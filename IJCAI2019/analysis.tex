...
\begin{theorem} \textbf{(Rank-1 Case)} \todob{No need to rub it in that this is rank $1$. This is the only theorem in the paper, right?}
...
\end{align*}
for any $\alpha > 0$ such that $U_{t}(i,1) > \alpha$ and $V_{t}(j,1) > \alpha$ \todoan{Both should be $\geq$} for all $i \in [K]$, $j \in [L]$, and $t \in [n]$.
...
\end{theorem}
\begin{proof}
Let, $(u_t v_t\transpose)_{t = 1}^n$ \todob{All $u$ and $v$ should be capitalized, no? At least consistent with the notation in the setting.} be a sequence of $n$ non-negative rank-$1$  matrices such that $u_t \in [0, 1]^{K \times 1}, v_t \in [0, 1]^{L \times 1}$, and the highest entry is $(1, 1)$. Let,
\begin{align*}
((i_t, j_t))_{t = 1}^n
\end{align*}
