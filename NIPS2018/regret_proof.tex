...
Let us assume that the users are coming in sequentially $i_1,...,i_n \in [K].$ We use the notation $j^*(i)$ to denote the optimal arm for user $i$. When $J =(j_1,...,j_k) \in [L]^k$ is a $k$-tuple, by $J[l]$ we will mean $j_l,$ the $l$'th entry of $J$ and  $\max R(i, J) := \max_l R(i,J[l]).$ %When the rewards $0 \leq R_t(i,j) \leq 1,$ we can assume with out loss of generality that $R_t(i,j) \in \{ 0,1\}$ is a Bernoulli random variable with appropriate parameter.  We will assume this henceforth. 
%and for a set of columns/arms $J \subset [L]$, by $J^*(i)$ we mean the best arm in $J$ for user $i$. 
...
\end{theorem}
We now prove the bounds for $R_c(n)$ and $R_r(n)$ separately. \myworries{Todo: Make it clear what the randomness is when using $\E$ throughout.}
