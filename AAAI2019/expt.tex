...
%Note, that at every timestep $t$, nature reveals an user $i_t$ and each algorithm suggests $d$ items to it and records all the $d$ feedbacks. \todob{Why do we need this reminding sentence?}
%\todob{American English is ``modeling''. Not ``modelling''.}

\todosb{I experimented with Round Robin only, do you want uniform randomly. Maybe 2nd expt we can show as unoform randomly}
