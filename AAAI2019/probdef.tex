...
% \todoan{Always use $\ll$ instead of $<<$}.  \todo{Changed}	
	
	Furthermore, we put a constraint on $V$ such that, $\forall j\in [L]$, $ \norm{V(j,:)}_1 \leq 1$. \todoan{Do we need this assumption??}
	\todosb{Need to ask Branislav. Didn't see this in proof} \todob{This is without loss of generality. It only affects the magnitudes of rewards.}
...
\todob{The above wording is very confusing because we have hott topics columns. I know that these are the rows of $V$.}
...
\textbf{Observation Model:} For each user $i_t$ revealed by the nature at round $t$, the learner is allowed to suggest atmost $d$-items, where $d$ is the rank of the matrix $M$. The user can click one, or all, or none of the recommendations and the learner observes all the $d$ clicks. We assume a Document Click Model (DCM) \citep{craswell2008experimental} such that every user-item pair has a single parameter called the user-item attraction factor which determines the click probability of the user when the item is shown. \todob{We do not do clicks anymore. The feedback is a non-negative number. You can interpret it as a preference, perhaps rating.} \todob{Look at the appendix to see how to state things more precisely.}

\textbf{Noise Model:}  At every timestep $t$, we generate a noisy matrix $\tilde{M}_t = UD_t V^{\intercal}$, \todob{Why does $M_t$ have a tilde?} where $D_t$ is a diagonal matrix such that $D_t(i,i)\in[0,1]$. Thus, for every such realization of $\tilde{M}_t$  the hott-topics structure of $M$ is preserved. 

%\todoan{remove $\forall t in [n]$, you are saying for every $t$ at the start of the paragraph}\todosb{removed $\forall t$}
...
\todoan{Describe our noise model first. Any discussions and comments justifying it should come after precisely explaining the observation and noise model} \todosb{added as discussion} \todob{We do not have a click model anymore. That was thrown out of window with the Bernoulli feedback.}
